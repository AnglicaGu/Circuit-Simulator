 ...
 * Name: Angelica Gu
 * Date: Jun 13 2025
 * Class: ExperimentController3
 * Description: Controller for ExperimentFrame3. Handles buttons, dragging
 * placing and check the tow circuits every time a new action is detected.
 */
...
public class ExperimentController3 implements ActionListener {

	// create the view
	private ExperimentFrame3 frame;

	// set a variable to store the situation of the circuit
	// 1=open circuit, 2=short circuit, 3=valid, 4=missing battery
	private int[] pathSituations = new int[2];
	private int finalSituation = 0;

	// store the audio of the buzzer
	private AudioClip buzzerClip = null;

	// constructor
	public ExperimentController3() {

		// view
		frame = new ExperimentFrame3();
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (GridCell cell : slots) {
					...
					if (bounds.contains(clicked) && !cell.isEmpty() && cell.getComponent() instanceof Switch sw) {
						...
						// update and check the three circuits again when every new component is put in
						updateAllPathSituations();
						finalValidCircuit();
						break;
					}
				}
	...
	// method to update the situation of every path
	// check the validation of each path
	private void updateAllPathSituations() {

		// set the two paths
		int[][] paths = { { 0, 2, 4, 5, 3, 1 }, { 0, 2, 6, 7, 3, 1 } };
		...
		for (int i = 0; i < paths.length; i++) {
			...
			// variable to record the situation of this path
			int[] path = paths[i];
			...
			boolean allSwitchOn = true;
			boolean hasNonWire = false;

			// loop throug each gird in a path
			for (int index : path) {

				// loop through all the grids
				GridCell cell = slots.get(index);
				...
				if (comp instanceof Switch sw && !sw.getIsOn())
					allSwitchOn = false;
			}

			// if there is an empty slot or open battery
			// situation is 1: open circuit
			if (hasEmpty || !allSwitchOn) {
				pathSituations[i] = 1;
			}

			// if there is no battery
			// situation is 4: missing battery
			else if (!hasBattery) {
				pathSituations[i] = 4;
			}
	...
	// decide the final situation based on the situation of each path
	private void finalValidCircuit() {

		// set and variable to see if there is a path that works itself
		// **parallel circuits are independent
		boolean has3 = false;

		// loop through all the path situations
		for (int situation : pathSituations) {

			// if there is a short circuit, the battery burns
			if (situation == 2) {

				// set the final situation 2: short circuit
				finalSituation = 2;
		...
		for (int situation : pathSituations) {
			...
			// if there is a circuit missing battery
			if (situation == 4) {

				// set final situation as default 1
				finalSituation = 1;
		...
		for (int situation : pathSituations) {
			...
			if (situation == 1) {
				...
				// enable the valid paths
				// **an open path doen't affect other pathes
				lightUpPathIfValid();
		...
		// if all correct, set the final situation 3: valid
		finalSituation = 3;

		// show ready panel
		showPanel(3);

		// light up all valid paths
		lightUpPathIfValid();
	...
	// method to enable independent valid pathes
	private void lightUpPathIfValid() {

		// 2D array to store the two pathes
		int[][] paths = { { 0, 2, 4, 5, 3, 1 }, { 0, 2, 6, 7, 3, 1 } };
		...
		for (int i = 0; i < paths.length; i++) {
			...
			if (pathSituations[i] == 3) {

				// loop through grids on this path
				for (int index : paths[i]) {
					Component comp = slots.get(index).getComponent();
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (e.getSource() == frame.warningButton1 || e.getSource() == frame.warningButton2) {
			frame.dispose();
			setVisible(false);
